...
local function executePythonFile(filePath)
    ...
    log(VERBOSE,
        string.format("runSimpleFile (%s): %s, %s", core.readString(filePathString), filePointer, filePathString))
...
try:
  ...
  win32api.MessageBox(None, f"{e}", "Error during aivmodel initialization", win32con.MB_OK)
        ...
        log(INFO, string.format("Test returned: %s", pythonCallbacks['onLordKilled'](0)))
